...
def get_triangulation(point_set_id: str):
    ...
    if r.status_code != 200:
        return jsonify(
            {
                "code": "UPSTREAM_ERROR",
                "message": f"Manager status {r.status_code}"
            }
        ), 503

    try:
        ps = PointSet.depuis_octets(r.content)
    except Exception as e:
        return jsonify(
            {
    ...
    tri = Triangulation.depuis_points_eventail(ps)
    binary = tri.vers_octets()
